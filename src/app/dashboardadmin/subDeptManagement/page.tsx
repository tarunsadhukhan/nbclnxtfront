...
export default function SubDeptManagement() {
  const [allBranches, setAllBranches] = useState<SubDept[]>([]);
  const [allSubDepartments, setAllSubDepartments] = useState<SubDept[]>([]);
  ...
  const createEditUrl = (dept: SubDept) => {
    const params = new URLSearchParams({
      subdeptId: dept.sub_dept_id.toString(),
    ...
    return `/dashboardadmin/subDeptManagement/createSubDept?${params.toString()}`;
  };
  ...
  const fetchAllBranches = async () => {
    ...
    try {
      ...
      if (storedBranchId && data.data.some((b: SubDept) => b.branch_id.toString() === storedBranchId)) {
  ...
  const fetchAllSubDepartments = async () => {
    ...
    try {
      const { data, error } = await fetchWithCookie(
        `${apiRoutesconsole.GET_SUBDEPARTMENT_ALL}`,
        "GET"
      );
      ...
      setAllSubDepartments(data.data || []);
    } catch (error) {
  ...
  const filteredDepartments = useMemo(() => {
    return allSubDepartments.filter(subdept => {
      // Filter by branch if one is selected
      const branchMatch =
        selectedBranchId === '' ||
        subdept.branch_id.toString() === selectedBranchId;

      // Filter by search query
      const searchMatch = subdept.dept_desc.toLowerCase().includes(searchQuery.toLowerCase());

      return branchMatch && searchMatch;
    });
  }, [allSubDepartments, selectedBranchId, searchQuery]);
      ...
      renderCell: (params) => (
        <div className="flex space-x-2">
          <Button
            ...
            size="icon"
            onClick={() => {
              const subdept = params.row as SubDept;
              if (typeof window !== "undefined") {
                
              const locaload: any = {
                co_id: subdept.co_id ? subdept.co_id : null,
                branch_id: subdept.branch_id ? subdept.branch_id : null,
            };
              ...
              window.location.href = createEditUrl(subdept);
            }}
          >
          ...
          <Button
            ...
            onClick={() => {
              const subdept = params.row as SubDept;
              if (confirm(`Are you sure you want to delete the department: ${subdept.sub_dept_desc}?`)) {
                // Call delete API or perform delete operation here
                console.log("Deleting department:", subdept.sub_dept_desc);
