...
import type { PackingMrSetup } from "../types/packingMrTypes";

// GET packing_mr_create_setup -> {data: {qualities, quality_groups,
//   readings_count}}. Cloned from useBeamMrSetup.
export function usePackingMrSetup(
	...
	React.useEffect(() => {
		...
		void fetchWithCookie<{ data: PackingMrSetup }>(url, "GET").then(({ data, error: err }) => {
			if (cancelled) return;
			if (err) {
				setError(err);
				setSetup(null);
