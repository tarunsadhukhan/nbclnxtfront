// Client-side preview for R-08-07A Inter Card & Tow Breaker (Card Sliver Weight).
// Mirrors the backend compute_card_sliver_stats. The server recomputes and
...
//
// DELTA vs the Breaker Card clone: the linkage is a BATCH, which has no single
// std, so the preview always uses DEFAULT_STD_MR_PCT (20, carding) and no CV band.
...
//
// A batch has no single std, so the preview always uses std 20 with NO CV band
