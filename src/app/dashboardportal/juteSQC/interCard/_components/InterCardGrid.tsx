...
type Props = {
	...
	grandAverages: InterCardGrandAverage[];
	loading: boolean;
	onDeleted: () => void;
};
...
type GridRow = InterCardReadingRow & { _gridId: string };
 ...
 * jute_sqc_card_sliver_wt.
 */
export default function InterCardGrid({
	coId,
	rows,
	sectionAverages,
	grandAverages,
	...
	const handleDelete = React.useCallback(
		async (id: number | null) => {
			...
			if (!confirm(`Delete card-sliver row #${id}?`)) return;
			const url = `${apiRoutesPortalMasters.CARD_SLIVER_WT_DELETE}/${id}?co_id=${coId}`;
			const { error } = await fetchWithCookie(url, "DELETE");
			if (error) {
				setSnack(error);
				return;
			}
			setSnack(`Deleted card-sliver row #${id}`);
			onDeleted();
		},
		[coId, onDeleted]
	);
	...
	const gridRows = React.useMemo<GridRow[]>(() => {
		...
		return sorted.map((r, i) => ({ ...r, _gridId: `${r.card_sliver_wt_id}-${i}` }));
	}, [rows]);
