...
const blankGridRow = (): PressProdGridRow => ({
  machine_id: "",
  quality_id: "",
  prod_qty: "",
  wk_hrs: "",
...
function isRowBlank(row: PressProdGridRow): boolean {
  return (
    row.machine_id === "" &&
    row.quality_id === "" &&
    row.prod_qty.trim() === "" &&
...
function hoursValid(row: PressProdGridRow): boolean {
...
/**
 * Create / edit dialog for Press Production entries — one spreadsheet-style
 ...
 * rows save individually (POST) or together via Save All. EDIT: the record
 * loads as a single editable grid row (no auto-extend) and Save PUTs it.
 * The rate shows read-only from the quality master; Div Hrs shows wk_hrs * 4.
 * The server resolves the real rate and computes amount/divisible hours.
 */
export default function CreatePressProductionPage({
  open,
  onClose,
  onSaved,
  editId,
  ...
  const [rows, setRows] = useState<PressProdGridRow[]>(() => [blankGridRow()]);
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      const { data, error } = await fetchWithCookie<{ data: PressProdSetup }>(
        `${apiRoutesPortalMasters.PRESS_PROD_SETUP}?${params}`,
        "GET",
      );
      if (cancelled) return;
      if (error || !data) {
  ...
  const loadRecord = useCallback(async () => {
    ...
    setLoading(true);
    try {
      const { data, error } = await fetchWithCookie<{ data: PressProdRecord }>(
        `${apiRoutesPortalMasters.PRESS_PROD_BY_ID}/${editId}`,
        "GET",
      );
      if (error || !data) throw new Error(error || "Failed to load press entry");
      const rec = data.data;
      ...
      setRows([
        {
          ...
          saved_id: rec.press_prod_id,
          dirty: false,
          remarks: rec.remarks ?? null,
        },
      ]);
    } catch (err: unknown) {
      notifyError(err instanceof Error ? err.message : "Error loading press entry");
    } finally {
      setLoading(false);
    }
  }, [editId, notifyError]);
  ...
  const setRowField = useCallback(
    <K extends keyof PressProdGridRow>(index: number, key: K, value: PressProdGridRow[K]) => {
      setRows((prev) => {
        const next = [...prev];
  ...
  const rowSaveable = (r: PressProdGridRow) =>
  ...
  const saveRowRequest = async (
    ...
    if (r.saved_id != null) {
      const { error } = await fetchWithCookie(
        `${apiRoutesPortalMasters.PRESS_PROD_EDIT}/${r.saved_id}`,
        "PUT",
        body,
      );
      return { id: r.saved_id, error: error ?? null };
    }
    const { data, error } = await fetchWithCookie<{ press_prod_id: number }>(
      apiRoutesPortalMasters.PRESS_PROD_CREATE,
      "POST",
      body,
    );
    return { id: data?.press_prod_id ?? null, error: error ?? null };
  };
  ...
  const title = isEdit ? "Edit Press Production" : "Create Press Production";
                ...
                >
                  ...
                  <TableBody>
                    {rows.map((r, i) => {
                      ...
                      // ponytail: divisible hrs = wk hrs * 4, mirrored from the DB generated column
                      const divHrs =
                        r.wk_hrs.trim() !== "" && Number.isFinite(wk) && wk >= 0 ? wk * 4 : null;
                      // Rate/hrs = amount / (div hrs - lost hrs), mirrored from the
                      // DB generated column (blank lost hrs counts as 0).
                      const qty = Number(r.prod_qty);
                      const amount =
                        quality?.quality_rate != null &&
                        r.prod_qty.trim() !== "" &&
                        Number.isFinite(qty) &&
                        qty > 0
                          ? Math.round(quality.quality_rate * qty * 100) / 100
                          : null;
                      const lost = r.lost_hrs.trim() === "" ? 0 : Number(r.lost_hrs);
                      const netHrs =
                        divHrs != null && Number.isFinite(lost) ? divHrs - lost : null;
                      const ratePerHrs =
                        amount != null && netHrs != null && netHrs > 0
                          ? Math.round((amount / netHrs) * 100) / 100
                          : null;
                      return (
                        <TableRow key={`press-row-${i}`}>
                          ...
                          <TableCell align="right" sx={cellSx}>
                            <Typography variant="body2" color="text.secondary" sx={{ fontSize: "0.875rem" }}>
                              {quality?.quality_rate != null
                                ? quality.quality_rate.toFixed(6)
                                : "—"}
                            </Typography>
                          </TableCell>
                          <TableCell align="right" sx={cellSx}>
