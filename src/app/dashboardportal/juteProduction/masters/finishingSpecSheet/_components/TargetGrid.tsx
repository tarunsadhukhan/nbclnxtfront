...
// --- Types (match the finishing target_map_grid contract exactly) ----------
// Finishing standards are three-dimensional: process (damping|calendering|…),
// id_type ('mcid' = machine | 'qid' = finishing quality) and value_role
// (standard | target | actual). 'actual' is captured on the Finishing SQC page,
// which reuses this same grid with the SQC actual endpoints. The grid itself is
// domain-agnostic — it renders exactly the params the backend returns.
...
// A dirty cell to send to the bulk-save endpoint. value === null clears a cell
// that previously held a value.
...
export default function TargetGrid({
	...
	// Reseed edits whenever a fresh grid loads (process / date / role change).
	React.useEffect(() => {
		setEdits(baseline);
	}, [baseline]);
	...
	if (grid.rows.length === 0) {
		return <Alert severity="info">No finishing machines / qualities found for this process.</Alert>;
	}

	if (grid.params.length === 0) {
		return <Alert severity="info">No parameters apply to this Process / Type / Role.</Alert>;
	}
				...
				<Table size="small" sx={{ minWidth: 600 }}>
					<TableHead>
						<TableRow>
							...
							<TableCell>Machine / Quality</TableCell>
							{grid.params.map((p) => (
