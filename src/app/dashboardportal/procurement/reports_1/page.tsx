...
import type { BranchOption } from "@/components/reports/types";
...
// returning via the Back button), not only the first page load.
let cachedReportNodes: ReportMenuNode[] | null = null;

export default function ProcurementReports1Page() {
  ...
  // DB-driven reports: direct report children (report=1) of this page's menu (920).
  ...
  useEffect(() => {
    let mounted = true;
    fetchReportMenuTree("procurement/reports_1")
      .then((res) => {
        if (!mounted) return;
  ...
  return (
    <Box sx={{ p: 3 }}>
      ...
      <Paper elevation={1} sx={{ mb: 3 }}>
        ...
        >
          {/* Searchable report picker — DB-driven (menu_mst report=1, children of menu 920). */}
          <Autocomplete
            options={dbReports}
